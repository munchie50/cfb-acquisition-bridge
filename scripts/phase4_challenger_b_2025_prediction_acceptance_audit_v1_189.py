...
assert len(S)==880 and len(T)==880 and set(S.game_id)==set(T.game_id)
assert S.game_id.nunique()==880 and T.game_id.nunique()==880
assert len(P)+len(E)==880 and not set(P.game_id)&set(E.game_id) and set(P.game_id)|set(E.game_id)==set(S.game_id)
assert (S.population_class=="FBS_VS_FBS").sum()==770 and (S.population_class=="FBS_VS_NONFBS").sum()==110
assert (S.competition_class=="REGULAR").sum()==839 and (S.competition_class=="CONFERENCE_CHAMPIONSHIP").sum()==10 and (S.competition_class=="POSTSEASON").sum()==31
