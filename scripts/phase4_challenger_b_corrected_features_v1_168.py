#!/usr/bin/env python3
# v1.168 corrected full mechanical + derived feature substrate, fail-closed on missing prior PBP primitives.
import sys,json
from pathlib import Path
import pandas as pd, pyreadr, numpy as np
hist,raw,old,out=map(Path,sys.argv[1:5]); out.mkdir(parents=True,exist_ok=True)
...
for y in range(2016,2025):
 ...
 dp=dp.merge(fd,on=["game_id","team"],how="outer")
...
for (y,team),z in mraw.sort_values(["season","team","start_date"]).groupby(["season","team"],sort=False):
 ...
 missing=(~z.mechanical_primitive_complete).astype(int).cumsum().shift(1,fill_value=0)
 ...
 "qualified_prior_games":prior,"missing_prior_mechanical_source_games":missing.astype(int),"mechanical_history_complete":hist_ok,
 ...
 "own_mechanical_primitive_complete":z.mechanical_primitive_complete})
...
for (y,team),z in draw.sort_values(["season","team","start_date"]).groupby(["season","team"],sort=False):
 z=z.sort_values("start_date").copy(); prior=np.arange(len(z)); idx=z.index
 def cs(c): return z[c].fillna(0).cumsum().shift(1,fill_value=0)
 missing=(~z.derived_primitive_complete).astype(int).cumsum().shift(1,fill_value=0); hist_ok=missing.eq(0)
 f=pd.DataFrame({"season":z.season,"game_id":z.game_id,"team":team,"start_date":z.start_date,"qualified_prior_games":prior,
 "missing_prior_derived_source_games":missing.astype(int),"derived_history_complete":hist_ok,
 ...
 "own_derived_primitive_complete":z.derived_primitive_complete})
...
ledger=mraw[key+["start_date","population_class","competition_class"]].merge(mf[key+["qualified_prior_games","missing_prior_mechanical_source_games","mechanical_history_complete","own_mechanical_primitive_complete"]],on=key).merge(df[key+["missing_prior_derived_source_games","derived_history_complete","own_derived_primitive_complete"]],on=key)
...
"opening_no_prior_team_sides":int(opening.sum()),"own_mechanical_primitive_missing_team_sides":int((~ledger.own_mechanical_primitive_complete).sum()),
