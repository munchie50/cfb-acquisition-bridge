#!/usr/bin/env python3
# v1.217: second rolling source/cutoff preflight only. No model prediction.
...
F.to_csv(outp/"challenger_b_2026_future_target_projection_v1_217.csv",index=False)
report={"status":"PASS_REFRESH_SOURCE_PREFLIGHT","cutoff_utc":cutoff.isoformat(),"schedule_sha256":sha(schedp),"pbp_sha256":sha(pbpp),"raw_schedule_rows_2026":len(R),"qualified_relevant_fbs_rows":len(Q),"spent_at_cutoff":len(spent),"future_at_cutoff":len(F),"future_fbs_vs_fbs":int((future.population_class=="FBS_VS_FBS").sum()),"future_fbs_vs_nonfbs":int((future.population_class=="FBS_VS_NONFBS").sum()),"completed_spent_games":len(spent_completed),"completed_spent_missing_pbp":len(missing_pbp),"missing_pbp_game_ids":missing_pbp,"membership_count":138,"model_prediction_performed":False,"market_joined":False,"target_outcomes_projected":False}
(outp/"preflight_v1_217.json").write_text(json.dumps(report,indent=2,sort_keys=True)+"\n")
