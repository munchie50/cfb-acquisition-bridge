...
DATA="challenger_b_dataset_v1_179.csv"
CFG="challenger_b_config_v1_179.json"
OUT="phase4_challenger_b_fit_v1_183"
...
for kind,tcol in targets.items():
    ...
    results[kind]={"selected_lambda":lam,"spent_corroborative":metrics(yv,p,kind)}
...
predout.to_csv(f"{OUT}/spent_corroborative_predictions.csv",index=False)
pd.DataFrame(cal).to_csv(f"{OUT}/win_calibration.csv",index=False)
pd.DataFrame(slices).to_csv(f"{OUT}/spent_corroborative_stability_slices.csv",index=False)
sc=pd.DataFrame({"feature":features,"mean":mu,"sd":sd}); sc.to_csv(f"{OUT}/train_scaling.csv",index=False)
summary={"status":"EXECUTED_NOT_ACCEPTED","train_rows":len(train),"spent_corroborative_rows":len(val),"selected":selected,"spent_corroborative":results,"lambda_grid":LAM,"fit_seasons":[2016,2017,2018,2019,2020,2021,2022],"spent_corroborative_seasons":[2023,2024],"test_2025_rows":int((df.season==2025).sum()),"unavailable_slices":["FBS_vs_FCS (classification metadata absent from frozen matrix)","conference (qualified conference metadata absent from frozen matrix)"],"software":{"numpy":np.__version__,"pandas":pd.__version__}}
