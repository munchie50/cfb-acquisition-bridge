...
S=json.load(open(scored/"summary_v1_196.json")); M=json.load(open(scored/"manifest_v1_196.json"))
...
for col in ["pred_margin","pred_total","pred_win"]:
    z=Q.set_index("game_id").loc[P.game_id,col].to_numpy(float); p=P[col].to_numpy(float)
    max_delta[col]=float(np.max(np.abs(z-p)))
    assert np.allclose(z,p,rtol=0,atol=1e-12), (col,max_delta[col])
for col in ["actual_margin","actual_total","actual_home_win"]:
    z=Q.set_index("game_id").loc[A.game_id,col].to_numpy(float)
    assert np.array_equal(z,A[col].to_numpy(float))
def gauss(y,p):
...
metrics={"margin":gauss(A.actual_margin.to_numpy(float),A.pred_margin.to_numpy(float)),"total":gauss(A.actual_total.to_numpy(float),A.pred_total.to_numpy(float)),"win":win(A.actual_home_win.to_numpy(float),A.pred_win.to_numpy(float))}
for k in metrics:
...
for fn,h in M.items(): assert hashlib.sha256((scored/fn).read_bytes()).hexdigest()==h["sha256"]
report={"status":"PASS","target_games":934,"scored_predictions":793,"frozen_exclusions":141,"target_key_exact":True,"outcomes_complete":True,"prediction_values_numerically_unchanged_at_1e_12":True,"prediction_max_serialization_delta":max_delta,"metrics_independently_recomputed":True,"scoring_artifact_hashes_reproduced":True,"metrics":metrics,"market_joined":False,"refit_performed":False,"recalibration_performed":False,"redesign_performed":False}
